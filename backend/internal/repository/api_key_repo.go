...
func (r *apiKeyRepository) withLoadedGroups(q *dbent.APIKeyQuery) *dbent.APIKeyQuery {
	return q.WithGroup().WithGroups(func(gq *dbent.GroupQuery) {
		...
		bg.Order(apikeygroup.ByPriority(), apikeygroup.ByGroupID()).WithGroup()
	})
}
...
func (r *apiKeyRepository) ListByUserID(ctx context.Context, userID int64, params pagination.PaginationParams, filters service.APIKeyListFilters) ([]service.APIKey, *pagination.PaginationResult, error) {
	...
	keysQuery := r.withLoadedGroups(q).
...
func (r *apiKeyRepository) ListAllByUserID(ctx context.Context, userID int64, filters service.APIKeyListFilters) ([]service.APIKey, error) {
	keys, err := r.withLoadedGroups(r.apiKeyListByUserIDQuery(userID, filters)).
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	if bindings := m.Edges.APIKeyGroups; len(bindings) > 0 {
		out.GroupIDs = make([]int64, 0, len(bindings))
		out.Groups = make([]service.Group, 0, len(bindings))
		for _, binding := range bindings {
			if binding == nil || binding.Edges.Group == nil {
				continue
			}
			group := binding.Edges.Group
			out.GroupIDs = append(out.GroupIDs, group.ID)
			if sg := groupEntityToService(group); sg != nil {
				out.Groups = append(out.Groups, *sg)
			}
		}
	} else if groups := m.Edges.Groups; len(groups) > 0 {
