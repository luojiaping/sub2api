...
// QueryAPIKeysMulti chains the current query on the "api_keys_multi" edge.
func (_q *GroupQuery) QueryAPIKeysMulti() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withAPIKeysMulti:      _q.withAPIKeysMulti.Clone(),
...
// WithAPIKeysMulti tells the query-builder to eager-load the nodes that are connected to
// the "api_keys_multi" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithAPIKeysMulti(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAPIKeysMulti = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAPIKeysMulti; query != nil {
		if err := _q.loadAPIKeysMulti(ctx, query, nodes,
			func(n *Group) { n.Edges.APIKeysMulti = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.APIKeysMulti = append(n.Edges.APIKeysMulti, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAPIKeysMulti(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.APIKeysMultiTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(group.APIKeysMultiPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.APIKeysMultiPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.APIKeysMultiPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "api_keys_multi" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
